...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed(), 2)/(w.Height/CmInM))*
...
func ReadData(training CaloriesCalculator) string {
	/*
		 Тут не вижу смысла считать каллории как по заданию так как во всех структурах TrainingInfo они уже посчитаны,
		 метод Calories() переопределен для Running, Swimming и Walking, поэтому они затеняют вложенную структуру Training.
